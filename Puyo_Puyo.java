...
public class Puyo_Puyo extends JFrame
{
	...
	private int prepareMove(int m){
		int r = this.rows;
		int c = this.cols;

		if(r*10+cols < this.puyo_len){
			return puyo_len*r+c;
		} else {
			return 0;
		}
	}
	public boolean initgame(int rows, int cols){
		int cont=0;
		for(int i=0; i<rows; i++){
			for(int j=0; j<cols; j++){
				int x = prepareMove(i*10+j);
				cont++;
			}
		}
			return (cont == this.rows+this.cols);
	}
	public Puyo_Puyo() 
	{
		super("Puyo_Puyo");
